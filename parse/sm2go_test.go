...
func TestWriteJson(t *testing.T) {
	...
	// state.json読み込み
	states, err := ioutil.ReadFile("./state.json")
	if err != nil {
		panic(err.Error())
	}

	var state_array []State
	err = json.Unmarshal(states, &state_array)
	if err != nil {
		panic(err.Error())
	}

	// event.json読み込み
	events, err := ioutil.ReadFile("./event.json")
	if err != nil {
		panic(err.Error())
	}

	var event_array []Event
	err = json.Unmarshal(events, &event_array)
	if err != nil {
		panic(err.Error())
	}

	// fmt.Println("state_array[0] is ", state_array[0])
