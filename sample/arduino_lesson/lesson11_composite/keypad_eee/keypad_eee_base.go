...
// 全体の領域と状態の中の領域の区別がついていない（Task関数の名称，現在の状態を表す変数currentStateの区別など）
...
var eodSmall Eod
var currentStateSmall State

func Task2() {
	if currentState == Off {
		// この領域を含む状態にいることが条件なので，「この状態下にいる時」という旨のif文が必要！
		switch currentStateSmall {
		case OffEmpty:
			if eodSmall == Entry {
				offemptyEntry()
				eodSmall = Do
